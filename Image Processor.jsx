...
function GlobalVariables() {
	...
	gFilesForCameraRaw = Array( "TIF", "CRW", "NEF", "RAF", "ORF", "MRW", "DCR", 
	...
	stractionDropDownLength = localize( "$$$/locale_specific/JavaScripts/ImageProcessor/ActionDropDownLength=165" );
...
function ImageProcessor() {
	...
	this.CreateDialog = function() {
		...
		var actionDropDownLength = StrToIntWithDefault( stractionDropDownLength, 165 );
		...
		d.line1 = l.add( 'panel', undefined, undefined,borderProperty);
		...
		d.etQuality = p.grpJPEG.grpLeft.grpQ.add( 'editnumber', undefined, undefined, 1, 12 );
		...
		d.etResizeJPEGW = p.grpJPEG.grpRight.grpW.add( 'editnumber', undefined, undefined, 1, gMaxResize);
		...
		d.etResizeJPEGH = p.grpJPEG.grpRight.grpH.add( 'editnumber', undefined, undefined, 1, gMaxResize);
		...
		p.line1 = p.add( 'panel', undefined, undefined,borderProperty);
		...
		d.etResizePSDW = p.grpPSD.grpRight.grpW.add( 'editnumber', undefined, undefined, 1, gMaxResize);
		...
		d.etResizePSDH = p.grpPSD.grpRight.grpH.add( 'editnumber', undefined, undefined, 1, gMaxResize);
		...
		p.line2 = p.add( 'panel', undefined, undefined,borderProperty);
		...
		d.etResizeTIFFW = p.grpTIFF.grpRight.grpW.add( 'editnumber', undefined, undefined, 1, gMaxResizeTIFF);
		...
		d.etResizeTIFFH = p.grpTIFF.grpRight.grpH.add( 'editnumber', undefined, undefined, 1, gMaxResizeTIFF);
		...
		d.ddSet.preferredSize.width = actionDropDownLength - 25;
		
		d.ddAction = l.grpDropDowns.add( 'dropdownlist' );
		d.ddAction.helpTip = strActionHelp;
		d.ddAction.preferredSize.width = actionDropDownLength + 25;

		d.hidden5 = l.grpAction.add( 'dropdownlist' );
	...
	this.InitDialog = function() {
		...
		d.etQuality.value = p["q"];
		d.cbMaximize.value = p["max"];
		d.cbResizeJPEG.value = p["jpegresize"];
		d.etResizeJPEGH.value = p["jpegh"];
		d.etResizeJPEGW.value = p["jpegw"];
		d.cbResizePSD.value = p["psdresize"];
		d.etResizePSDH.value = p["psdh"];
		d.etResizePSDW.value = p["psdw"];
		d.cbResizeTIFF.value = p["tiffresize"];
		d.etResizeTIFFH.value = p["tiffh"];
		d.etResizeTIFFW.value = p["tiffw"];
	...
	this.GetParamsFromDialog = function() {
		...
		p["q"] = d.etQuality.value;
		p["max"] = d.cbMaximize.value;
		p["jpegresize"] = d.cbResizeJPEG.value;
		p["jpegh"] = d.etResizeJPEGH.value;
		p["jpegw"] = d.etResizeJPEGW.value;
		p["psdresize"] = d.cbResizePSD.value;
		p["psdh"] = d.etResizePSDH.value;
		p["psdw"] = d.etResizePSDW.value;
		p["tiffresize"] = d.cbResizeTIFF.value;
		p["tiffh"] = d.etResizeTIFFH.value;
		p["tiffw"] = d.etResizeTIFFW.value;
		...
		this.dlgMain.btnRun.onClick = function () {
			...
			if ( d.cbJPEG.value ) {
			    ...
			    if ( d.cbResizeJPEG.value ) {
				    var w = d.etResizeJPEGW.value;
				    var h = d.etResizeJPEGH.value;
			...
			if ( d.cbTIFF.value && d.cbResizeTIFF.value ) {
				var w = d.etResizeTIFFW.value;
				var h = d.etResizeTIFFH.value;
			...
			if ( d.cbPSD.value && d.cbResizePSD.value ) {
				var w = d.etResizePSDW.value;
				var h = d.etResizePSDH.value;
			    if ( isNaN( w ) || w < 1 || w > gMaxResize ||
			         isNaN( h ) || h < 1 || h > gMaxResize ) {
					alert( strPSDWandH );
					return;
				...
				if ( ( inputFiles[i] instanceof File && 
					...
					if ( this.runningFromBridge ) {
						...
						try {
							...
							// eat excpetions thrown by image not backed by a file and use the name as the fileName
							// however, if the user selects saveinsame, then we can't proceed since there is no "same"
							// path. similarly, if the document's name is empty, we dont have a filename to write to
							if (this.params["saveinsame"] || inputFiles[i].name.length == 0) {
								throw (strMustSaveOpen)
							}
							cameraRawParams.fileName = inputFiles[i].name
						}
					} else {
						cameraRawParams.fileName = inputFiles[i].absoluteURI.toString();
						// Change the source path to an URI.
					...
					if (this.params["useopen"] ||
						IsFileOneOfThese( cameraRawParams.fileName, gFileExtensionsToRead ) ||
